...
get() {
    ...
    protocol)
        ...
        socks*)
            net=socks
            is_protocol=$net
            [[ ! $is_socks_user ]] && is_socks_user=mysb
...
info() {
    ...
    ws | tcp | h2 | quic | http*)
        if [[ $host ]]; then
            ...
            [[ $is_protocol == 'vmess' ]] && {
                is_vmess_url=$(jq -c '{v:2,ps:'\"mysb-$net-$host\"',add:'\"$is_addr\"',port:'\"$is_https_port\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',host:'\"$host\"',path:'\"$path\"',tls:'\"tls\"'}' <<<{})
                ...
                is_url="$is_protocol://$uuid@$host:$is_https_port?encryption=none&security=tls&type=$net&host=$host&path=$path#mysb-$net-$host"
            }
            ...
            is_vmess_url=$(jq -c "{v:2,ps:\"mysb-${net}-$is_addr\",add:\"$is_addr\",port:\"$port\",id:\"$uuid\",aid:\"0\",net:\"$net\",type:\"$is_type\"$is_quic_add}" <<<{})
            is_url=vmess://$(echo -n $is_vmess_url | base64 -w 0)
        fi
        ;;
    ss)
        ...
        is_url="ss://$(echo -n ${ss_method}:${ss_password} | base64 -w 0)@${is_addr}:${port}#mysb-$net-${is_addr}"
    ...
    trojan)
        ...
        is_url="$is_protocol://$password@$is_addr:$port?type=tcp&security=tls&insecure=1&allowInsecure=1#mysb-$net-$is_addr"
    ...
    hy*)
        ...
        is_url="$is_protocol://$password@$is_addr:$port?alpn=h3&insecure=1&allowInsecure=1&pinSHA256=$is_sha256#mysb-$net-$is_addr"
    ...
    tuic)
        ...
        is_url="$is_protocol://$uuid:$password@$is_addr:$port?alpn=h3&insecure=1&allowInsecure=1&congestion_control=bbr#mysb-$net-$is_addr"
    ...
    reality)
        ...
        is_url="$is_protocol://$uuid@$is_addr:$port?encryption=none&security=reality&flow=$is_flow&type=$is_net_type&sni=$is_servername&pbk=$is_public_key&fp=chrome#mysb-$net-$is_addr"
        ;;
    anytls)
        is_can_change=(0 1 4)
        if [[ $is_anytls_domain ]]; then
            ...
            is_url="anytls://$password@$is_anytls_domain:$port#mysb-$net-$is_anytls_domain"
        else
            is_insecure=1
            is_info_show=(0 1 2 10 8 20)
            is_info_str=($is_protocol $is_addr $port $password tls true)
            is_url="anytls://$password@$is_addr:$port?insecure=1&allowInsecure=1#mysb-$net-$is_addr"
        fi
        ;;
    direct)
    ...
    socks)
        ...
        is_url="socks://$(echo -n ${is_socks_user}:${is_socks_pass} | base64 -w 0)@${is_addr}:${port}#mysb-$net-${is_addr}"
        ;;
    esac
    ...
    if [[ $is_new_install ]]; then
        warn "首次安装完成, 可使用 sb help 查看使用帮助."
    fi
    if [[ $is_url ]]; then
        msg "------------- ${info_list[12]} -------------"
